...
class _MessageRowState extends ConsumerState<MessageRow> {
              ...
              children: [
                ...
                ConstrainedBox(
                  ...
                  child: Row(
                    ...
                    children: [
                      Flexible(
                        // Every `@nym` mention (here and everywhere) is decorated
                        // with the mentioned user's inline avatar + flair by
                        // `_MentionChip` (PWA `_enrichActionMentions`,
                        // messages.js:1369-1403).
  ...
  Widget _readerAvatars(BuildContext context) {
    ...
    final c = context.nym;
    final users = ref.watch(usersProvider);
    // The modal anchors to the render object of whatever context it is handed,
    // so that context has to belong to the AVATARS and nothing more. The outer
    // Row is MainAxisSize.max — it spans the full width to right-align — so
    // wrapping it would anchor the sheet to a full-width box and open it
    // visibly offset from the avatars that were pressed. The Builder therefore
    // sits INSIDE that Row, around a MainAxisSize.min row of just the avatars.
    return Padding(
          ...
          Builder(
            builder: (avatarContext) => GestureDetector(
              onLongPress: () {
                ...
                // reader long-press fires (groups.js:2759-2763, 2806-2810). A
                // real 30ms motor pulse reads as a solid tap — mediumImpact,
                // not the barely perceptible lightImpact.
