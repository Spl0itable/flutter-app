...
void main() {
  group('parse cache memoization', () {
    ...
    test('different content is not confused across cache entries', () {
      ...
      expect((a.first as ParagraphBlock).inlines.whereType<BoldNode>(),
          hasLength(1));
      expect((b.first as ParagraphBlock).inlines.whereType<ItalicNode>(),
          hasLength(1));
    });
    ...
    test('different customEmojis identity produces a cache miss (fresh parse)',
      ...
      final ctx1 =
          FormatContext(customEmojis: const {'smile': 'https://x/1.png'});
      ...
      expect(identical(r1, r2), isFalse,
          reason:
              'a different emoji map instance must not reuse the cached parse');
    });
    ...
    test(
        'cache is bounded (no unbounded growth) — many uniques still parse correctly',
        () {
      NymFormat.clearParseCache();
      ...
      expect((again.first as ParagraphBlock).inlines.whereType<ItalicNode>(),
          hasLength(1));
    });
  });
}
